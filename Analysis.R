# Analysis.R — final (PDF-only) -> writes plots_2321.pdf
# Author: Sarvadhnya Patil (2321)
# Usage: source("Analysis.R") in RStudio (interactive) or Rscript Analysis.R

# ---------------- Packages ----------------
required <- c("readr","dplyr","ggplot2","scales","stringr","ggrep2","ggrepel","tidyr","RColorBrewer","rstudioapi")
# note: ggpr ep? user had ggrepel earlier; ensure required contains ggrepel only
...
library(readr); library(dplyr); library(ggplot2); library(scales); library(stringr)
library(ggrepel); library(tidyr); library(RColorBrewer)
...
if (interactive() && requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::hasFun("getActiveDocumentContext")) {
  ...
  if (nzchar(script_path)) setwd(dirname(script_path))
}
...
out_pdf  <- file.path(getwd(), "plots_2321.pdf")
TOP_N    <- 10
SCATTER_N<- 50
...
# 8) Pie — WHO region share (no slice labels; legend "region — %"; sorted; white borders)
p_pie <- (function() {
...
# ---------------- Show interactively ----------------
plots <- list(
  p_bar_confirmed, p_bar_deaths, p_scatter, p_hist,
  p_bar_recovered, p_bar_active, p_line_metrics, p_pie
)

for (p in plots) if (!is.null(p)) show_plot_interactive(p)

# ---------------- Robust PDF write ----------------
options(bitmapType = "cairo")  # prefer cairo rendering

# try to remove existing PDF (avoid file lock)
if (file.exists(out_pdf)) {
  tryCatch({
    file.remove(out_pdf)
  }, error = function(e) {
    message("Warning: could not remove existing PDF (maybe open). Proceeding to attempt writing anyway.")
  })
}

# open device (try cairo_pdf then fallback to pdf)
used_device <- "cairo_pdf"
opened <- FALSE
try({
  cairo_pdf(filename = out_pdf, width = 11, height = 8.5)
  opened <- TRUE
}, silent = TRUE)
...
if (!opened) stop("Unable to open a PDF device to write ", out_pdf)

message("Writing combined PDF (device = ", used_device, "): ", out_pdf)
...
for (i in seq_along(plots)) {
  ...
  tryCatch({
    ...
    message("Failed to render plot ", i, " to PDF: ", e$message)
  })
}

dev.off()
message("Combined PDF written: ", out_pdf)
